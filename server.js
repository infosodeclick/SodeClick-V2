...
function renderBoardPage(me, q = {}, info = '') {
  ...
  const categories = ['all', 'general', 'dating', 'friend', 'tips', 'event'];
  ...
  const feed = filtered.map((p) => {
    ...
    const avatar = (p.author || '?').slice(0, 1).toUpperCase();
    const image = p.imageUrl ? `<div style="margin-top:10px"><img src="${p.imageUrl}" alt="post" style="width:100%;max-height:520px;object-fit:cover;border-radius:12px;border:1px solid #f3dce8"/></div>` : '';
    const commentsHtml = comments.slice(-5).map((c) => `<div class="muted" style="margin-top:6px">• <strong>${c.by}</strong>: ${c.text}</div>`).join('');

    return `
      <article class="card" style="padding:12px;border-radius:12px">
        <div style="display:flex;gap:10px;align-items:flex-start">
          <div style="width:42px;height:42px;border-radius:999px;background:#fde7f2;border:1px solid #f9a8d4;display:flex;align-items:center;justify-content:center;font-weight:800">${avatar}</div>
          <div style="flex:1;min-width:0">
            <div style="display:flex;justify-content:space-between;gap:8px;align-items:center;flex-wrap:wrap">
              <strong>${p.pinned ? '📌 ' : ''}${p.author}</strong>
              <span class="muted" style="font-size:12px">${new Date(p.createdAt).toLocaleString('th-TH')}</span>
            </div>
  ...
  return htmlPage('Webboard', `
    <main class="card" style="display:grid;gap:12px;max-width:900px;margin:0 auto">
      ...
      <section class="card" style="padding:12px;border-radius:12px;display:grid;gap:8px">
        <form method="POST" action="/board/new" style="display:grid;gap:8px">
          <div><input name="title" placeholder="หัวข้อโพสต์ (ไม่ใส่ก็ได้)" /></div>
          <div><textarea name="content" rows="4" placeholder="คุณกำลังคิดอะไรอยู่..."></textarea></div>
          <div class="grid">
            <div><label>หมวดหมู่</label><select name="category">${categoryOptions}</select></div>
            <div><label>ลิงก์รูป (ตัวเลือก)</label><input name="imageUrl" placeholder="https://.../photo.jpg"/></div>
          </div>
          <div style="display:flex;justify-content:flex-end"><button class="btn btn-primary" type="submit">โพสต์</button></div>
        </form>
      </section>

      <section class="card" style="padding:12px;border-radius:12px">
        <form method="GET" action="/board" style="display:flex;gap:8px;flex-wrap:wrap;align-items:flex-end">
          <div style="min-width:160px"><label>หมวดหมู่</label><select name="category"><option value="all">ทั้งหมด</option>${categoryOptions}</select></div>
          <div style="min-width:220px;flex:1"><label>ค้นหาโพสต์</label><input name="search" value="${q.search || ''}" placeholder="ค้นหาโพสต์..."/></div>
          <div><button class="btn" type="submit">ค้นหา</button></div>
        </form>
      </section>

      <section style="display:grid;gap:10px">${feed || '<div class="muted">ยังไม่มีโพสต์</div>'}</section>
    </main>
  `);
}
